...
$statement = $product->allCateg($ecat_id);
$total = $statement->rowCount();
$result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
foreach ($result as $row) {
...
$product->UpdateView($p_total_view,$_REQUEST['id']);


$statement = $product->GetSize($_REQUEST['id']);
$result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
foreach ($result as $row) {
    $size[] = $row['size_id'];
}

$statement = $product->GetColor($_REQUEST['id']);
$result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
foreach ($result as $row) {
    $color[] = $row['color_id'];
}
...
if(isset($_POST['form_review'])) {
    ...
    $statement = $Rating->GetPerCust($_REQUEST['id'],$_SESSION['customer']['cust_id']);
    $total = $statement->rowCount();
    
    if($total) {
        $error_message = LANG_VALUE_68; 
    } else {
         $Rating->insert($_REQUEST['id'],$_SESSION['customer']['cust_id'],$_POST['comment'],$_POST['rating']);
...
$t_rating = 0;
$statement = $Rating->Get($_REQUEST['id']);
					...
					<div class="row">
						<div class="col-md-5">
							<ul class="prod-slider">
                                ...
                                <?php
                                $statement = $product-> GetPhotos($_REQUEST['id']);
                                $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                                foreach ($result as $row) {
                                    ?>
							...
							<div class="tab-content">
								...
								<div role="tabpanel" class="tab-pane" id="review" style="margin-top: -30px;">
                                    ...
                                    <div class="review-form">
                                        <?php
                                        $statement = $Rating->allJoinCust($_REQUEST['id']);
                                        $total = $statement->rowCount();
                                        ?>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="product-carousel">
                    ...
                    <?php
                    $statement = $product->GetOtherProduct($ecat_id,$_REQUEST['id']);
                    $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                    foreach ($result as $row) {
                        ?>
                        <div class="item">
                            ...
                            <div class="text">
                                ...
                                <div class="rating">
                                    <?php
                                    $t_rating = 0;
                                    $statement1 = $Rating->Get($row['p_id']);
