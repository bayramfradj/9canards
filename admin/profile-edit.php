	...
	if($_SESSION['user']['role'] == 'Super Admin') {
	    ...
	    if(empty($_POST['email'])) {
	    	...
	    	if (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false) {
		    	...
		    	$statement = $User->GetUser($_SESSION['user']['id']);
				$result = $statement->fetchAll(PDO::FETCH_ASSOC);
				foreach($result as $row) {
					$current_email = $row['email'];
				}

		    	$statement = $User->cheekMail($_POST['email'],$current_email);
	    ...
	    if($valid == 1) {
			...
			$User->update1(array($_POST['full_name'],$_POST['email'],$_POST['phone'],$_SESSION['user']['id']));
		...
		// updating the database
		$User->updatePhone(array($_POST['phone'],$_SESSION['user']['id']));
...
if(isset($_POST['form2'])) {
    ...
    if($valid == 1) {
        ...
        // updating the database
		$User->updatePhoto(array($final_name,$_SESSION['user']['id']));
...
if(isset($_POST['form3'])) {
    ...
    if( md5($_POST['cpassword']) != $_SESSION['user']['password']  ) {
        $valid = 0;
        $error_message .= "current Password False <br>";
    }
    ...
    if($valid == 1) {
    	...
    	// updating the database
		$User->updatePassword(array(md5($_POST['password']),$_SESSION['user']['id']));
