...
if(isset($_POST['form1'])) {
	...
	if(empty($_POST['post_title'])) { 
		...
		// Duplicate Checking,
    	$statement = $Post-> GetPostWithTitle($_POST['post_title']);
	...
	if($valid == 1) {
		...
		$statement = $Post->GetPostWithSlug($post_slug);
		$total = $statement->rowCount();
		if($total) {
		...
		if($path=='') {
			...
			$Post->insert(array($_POST['post_title'],$post_slug,$_POST['post_content'],$_POST['post_date'],'',$_POST['category_id'],0,$_POST['meta_title'],$_POST['meta_keyword'],$_POST['meta_description']));
		} else {
    		// uploading the photo into the main location and giving it a final name
    		$final_name = 'post-'.$ai_id.'.'.$ext;
            move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );

            $Post->insert2(array($_POST['post_title'],$post_slug,$_POST['post_content'],$_POST['post_date'],$final_name,$_POST['category_id'],0,$_POST['meta_title'],$_POST['meta_keyword'],$_POST['meta_description']));
		}
