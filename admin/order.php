...
if(isset($_POST['form1'])) { 
    ...
    if($valid == 1) {
        ...
        $order_detail = '';
        $statement = $Payment->GetByP_Id($_POST['payment_id']);
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
        foreach ($result as $row) {
...
Payment Id: '.$row['payment_id'].'<br>
        ...
        $i=0;
        $statement = $Orders->ListOrdersWithPayment($_POST['payment_id']);
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
        foreach ($result as $row) {
            $i++;
            $order_detail .= '
...
<section class="content-header">
	<div class="content-header-left">
		<h1>View Orders paid with paypal</h1>
	</div>
</section>
