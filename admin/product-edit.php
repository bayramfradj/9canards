    ...
    if($valid == 1) {
    	...
    	if( isset($_FILES['photo']["name"]) && isset($_FILES['photo']["tmp_name"]) )
        {
            ...
            if(isset($final_name1)) {
		        ...
		        {
		        	$Product->insertPhoto($final_name1[$i],$_REQUEST['id']);
		        }
            }            
        }
        ...
        if(isset($_POST['size'])) {
			...
			foreach($_POST['size'] as $value) {
				$Product->insertSize($value,$_REQUEST['id']);
			}
		} else {
			$Product->deleteSize($_REQUEST['id']);
		}
		...
		if(isset($_POST['color'])) {
			...
			foreach($_POST['color'] as $value) {
				$Product->insertColor($value,$_REQUEST['id']);
			}
		} else {
			$Product->deleteColor($_REQUEST['id']);
		}
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select name="mcat_id" class="form-control select2 mid-cat">
		                            ...
		                            <?php
		                            $statement = $MidCategory->ListWithTCat($tcat_id);
		                            $result = $statement->fetchAll(PDO::FETCH_ASSOC);   
		                            foreach ($result as $row) {
		                                ?>
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select name="ecat_id" class="form-control select2 end-cat">
		                            ...
		                            <?php
		                            $statement = $EndCategory->all3($mcat_id);
		                            $result = $statement->fetchAll(PDO::FETCH_ASSOC);   
		                            foreach ($result as $row) {
		                                ?>
