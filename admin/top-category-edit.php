...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['tcat_name'])) {
    	...
    	$statement = $TopCategory->Get($_REQUEST['id']);
		$result = $statement->fetchAll(PDO::FETCH_ASSOC);
		foreach($result as $row) {
			$current_tcat_name = $row['tcat_name']; 
		}

		$statement = $TopCategory->checkName(array($_POST['tcat_name'],$current_tcat_name));
    ...
    if($valid == 1) {    	
		...
		$TopCategory->update(array($_POST['tcat_name'],$_POST['show_on_menu'],$_REQUEST['id']));
